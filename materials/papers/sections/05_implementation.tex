...
The platform runs on k3s on a single node \cite{kubernetes2024}. The node count is an implementation choice, and moving to a multi-node cluster should change only the Kustomize overlay, though that move has not been performed. Library versions and base images are pinned to the highest stable release as of April 2026 so that testing is reproducible, and a component is intended to be replaceable by an equivalent without changing the design.
