...
\hline
SK-F-01 & KF-01 & Mengubah definisi \textit{feature view} pada repositori Gitea lalu menunggu rekonsiliasi Argo CD, kemudian memverifikasi \texttt{feast feature-views list}. & \textit{Feature view} baru aktif tanpa intervensi manual dan perubahan terlacak pada riwayat Git. \\
\hline
SK-F-06 & KF-06 & Mempromosikan model melalui Argo Rollouts dengan strategi kanari, lalu menyuntikkan model bermasalah untuk memicu \textit{rollback}. & Model valid dipromosikan; model bermasalah memicu \textit{rollback} otomatis tanpa kehilangan permintaan. \\
\hline
...
\hline
SK-F-13 & KF-13 & Menjalankan satu \textit{notebook} UV yang memanggil SDK Feast, MLflow, dan klien KServe v2 secara berurutan. & Ketiga SDK dipanggil dari satu sesi tanpa konfigurasi tambahan per layanan. \\
\hline
...
\hline
SK-N-12 & KNF-12 & Menjalankan \texttt{make test} untuk cakupan uji dan \texttt{make phase-full} pada \textit{cluster} bersih sambil mengukur waktu konvergensi. & Cakupan uji melewati ambang yang ditetapkan dan \texttt{phase-full} konvergen tanpa langkah manual. \\
\hline
...
\hline
SK-F-09 & KF-09 & Menjalankan dua \textit{run} pelatihan pada MLflow, lalu memutar ulang \textit{run} dari \textit{snapshot} data dan parameter tercatat. & \textit{Run} tercatat lengkap dan pemutaran ulang menghasilkan selisih metrik di bawah satu persen. \\
\hline
...
\hline
SK-N-05 & KNF-05 & Menjalankan \textit{checkpoint} Great Expectations dan menyuntikkan probe \textit{timestamp} masa depan untuk menguji kebocoran temporal. & Pelanggaran kontrak kualitas terdeteksi dan tidak ada kebocoran data temporal yang lolos. \\
\hline
...
\hline
SK-F-05 & KF-05 & Menjalankan \textit{pipeline} pelatihan pada Kubeflow Pipelines yang membaca fitur dari Feast dan menulis model ke MLflow. & \textit{Pipeline} berjalan dari ujung ke ujung dan model terdaftar pada registri MLflow. \\
\hline
SK-F-07 & KF-07 & Menyuntikkan \textit{drift} sintetis pada satu fitur, lalu mengamati detektor \textit{drift} dan \textit{CronWorkflow retrain-on-drift}. & \textit{Drift} terdeteksi saat ambang PSI atau KS terlampaui dan \textit{pipeline} pelatihan ulang terpicu otomatis. \\
\hline

\multicolumn{4}{|l|}{\textbf{Tujuan T-4: layanan fitur \textit{dual-store}}} \\
\hline
SK-F-02 & KF-02 & Membandingkan nilai fitur \textit{online} (Valkey) dan \textit{offline} (ClickHouse) untuk \textit{entity} yang sama. & Nilai \textit{online} dan \textit{offline} konsisten untuk \textit{entity} dan waktu acuan yang sama. \\
\hline
SK-F-04 & KF-04 & Menjalankan beban pencarian vektor pada Qdrant melalui \texttt{load/vector-search-latency.js} dengan k6. & \textit{Recall} memenuhi target dan latensi pencarian berada pada orde sub-detik. \\
\hline
...
\hline
SK-N-09 & KNF-09 & Mengakses seluruh konsol platform melalui satu sesi identitas dex serta memeriksa ketersediaan dokumentasi operasional pada repositori Gitea. & Seluruh konsol dapat dicapai melalui satu jalur autentikasi tunggal tanpa login ulang dan dokumentasi prosedur inti tersedia. \\
\hline
