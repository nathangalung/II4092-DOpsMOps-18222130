% Konfigurasi \textit{ingestion pipeline} Kafka pada V.2.1 dalam bentuk tabel agar
...
\begin{table}[!htb]
\centering
\caption{Konfigurasi \textit{Ingress} Kafka pada Lingkungan Implementasi}
...
\hline
Protokol klien & SASL\_SSL dengan SCRAM-SHA-512, kredensial dari OpenBao melalui External Secrets Operator & Autentikasi dan enkripsi seragam tanpa \textit{secret} pada manifes \\
\hline
...
\hline
Topologi \textit{topic} & Enam \textit{partition}, faktor replikasi satu, dinaikkan tiga pada \textit{multi-node} & Paralelisme konsumen tanpa melampaui kapasitas satu \textit{node} \\
\hline
\textit{Scaling} konsumen & KEDA \texttt{ScaledObject} membaca \textit{lag} dari Kafka Exporter melalui Prometheus & Skala konsumen mengikuti panjang antrean nyata \\
\hline
