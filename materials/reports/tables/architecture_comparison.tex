...
\begin{longtable}{|m{2cm}|m{2.7cm}|m{2.7cm}|m{3.05cm}|m{2cm}|}
\caption{Perbandingan Detail Arsitektur Saat Ini dengan yang Diusulkan}
\label{tab:architecture_comparison} \\
\hline
\multicolumn{1}{|>{\centering\arraybackslash}m{2cm}|}{\textbf{Aspek}} &
\multicolumn{1}{>{\centering\arraybackslash}m{2.7cm}|}{\textbf{Arsitektur Saat Ini}} &
\multicolumn{1}{>{\centering\arraybackslash}m{2.7cm}|}{\textbf{Arsitektur Diusulkan}} &
\multicolumn{1}{>{\centering\arraybackslash}m{3.05cm}|}{\textbf{Manfaat yang Diharapkan}} &
...
\textcite{lakka2025kubernetes} menunjukkan bahwa adopsi Kubernetes pada perusahaan disertai praktik DevOps yang matang meningkatkan \textit{deployment frequency} dan menekan \textit{Mean Time To Recovery} (MTTR) secara signifikan, dan arsitektur yang diusulkan menargetkan karakteristik performa pada arah yang sama. &
...
Duplikasi fitur dapat ditekan, \textit{training-serving skew} berkurang melalui pemisahan \textit{offline} dan \textit{online} serta \textit{point-in-time retrieval}, dan \textit{feature reuse} berpotensi meningkat signifikan. &
...
Qdrant sebagai indeks vektor terpisah dari \textit{online store} tabular Valkey, dengan HNSW \textit{native}, API gRPC, dan filter \textit{payload}. &
\textcite{johnson2021billion} menunjukkan bahwa indeks vektor berbasis FAISS dapat melayani \textit{billion-scale similarity search} dengan latensi rendah, sedangkan \textcite{malkov2020efficient} menunjukkan \textit{approximate nearest neighbor search} yang efisien pada indeks HNSW. Desain ini memakai Qdrant agar fitur \textit{embedding} dilayani secara \textit{online} tanpa membebani \textit{online store} \textit{key-value} dan tanpa membangun layanan kustom. &
\citereset\autocite{johnson2021billion,malkov2020efficient,qdrant2025} \\
\hline
Validitas Temporal &
...
Pendekatan ini mencegah kebocoran data temporal yang dapat menghasilkan estimasi performa yang terlalu optimistis saat evaluasi dan tidak realistis di produksi. &
...
Spark sebagai mesin \textit{batch processing} dan Flink sebagai mesin \textit{stream processing} yang hasilnya dimaterialisasikan langsung ke \textit{Feature Store}. Airflow mengorkestrasi \textit{pipeline} transformasi data sebagai DAG sehingga pengembangan dan penjadwalan transformasi menjadi lebih sistematis. &
...
\textit{Workflow} \textit{training} berbasis \textit{notebook} dengan \textit{tracking} eksperimen informal dan \textit{artifact} tidak terkelola. &
Kubeflow \textit{Pipelines} untuk orkestrasi \textit{training} dan MLflow untuk \textit{tracking} eksperimen. &
Reproduksibilitas meningkat melalui kontainerisasi dan pengelolaan eksperimen terpusat, sementara siklus pengembangan \textit{pipeline} menjadi lebih terstruktur. &
...
KServe dengan \textit{InferenceService} deklaratif dan \textit{deployment} otomatis melalui GitOps di atas Knative Serving. OpenTelemetry mengumpulkan log dan \textit{trace} \textit{inference} ke Loki dan Grafana Tempo sebagai dasar pemantauan dan analitik lanjutan. &
...
\hline
Tata Kelola \& \textit{Lineage} &
Dokumentasi dilakukan secara manual dan \textit{audit trail} sering tidak lengkap atau tersebar. &
DataHub dengan \textit{metadata ingestion} otomatis dan \textit{lineage} yang dapat ditelusuri dari \textit{source} hingga model, dibantu OpenLineage sebagai protokol pengirim \textit{event} lintas \textit{engine} pemrosesan. &
...
\hline
\textit{Observability} &
Fokus pemantauan pada metrik infrastruktur, tanpa cakupan metrik ML spesifik di sebagian besar organisasi. &
...
\hline
Integrasi CI/CD &
CI/CD untuk \textit{pipeline} dan model dilakukan secara manual atau semiotomatis, dengan alur kerja yang tidak seragam. &
GitOps dengan Argo CD sebagai mekanisme \textit{continuous deployment} deklaratif, dilengkapi Tekton untuk \textit{continuous integration} dan Argo Rollouts untuk \textit{progressive delivery}, dengan Git sebagai \textit{single source of truth} yang mencakup seluruh konfigurasi DataOps dan MLOps. &
...
\textit{Developer} menghabiskan banyak waktu untuk tugas repetitif, \textit{onboarding} lama, dan penggunaan \textit{tools} yang terpisah. &
Platform terintegrasi dengan \textit{self-service} untuk data, fitur, \textit{pipeline}, dan akses. Dex sebagai \textit{identity provider} OIDC dan oauth2-proxy menyatukan autentikasi tunggal di seluruh antarmuka pengembang. &
...
\textit{Auto-scaling} berbasis HPA, VPA, dan KEDA, pemantauan biaya melalui OpenCost, serta alokasi sumber daya yang lebih terukur pada \textit{cluster} Kubernetes. Seluruh komponen inti berbasis \textit{open source} sehingga biaya lisensi dapat ditekan. &
...
RBAC terintegrasi, \textit{audit trail} komprehensif, dan pengecekan \textit{compliance} yang terotomatisasi. Dex sebagai \textit{identity provider} OIDC memusatkan identitas, oauth2-proxy menegakkan autentikasi pada antarmuka aplikasi, Kyverno menegakkan kebijakan admisi pada \textit{cluster}, OpenBao mengelola \textit{secrets}, sementara Falco dan Trivy Operator memantau ancaman \textit{runtime} dan kerentanan \textit{image}. &
