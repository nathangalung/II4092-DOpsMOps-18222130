...
\hline
SK-F-06 & KF-06 & Mempromosikan model melalui Argo Rollouts dengan strategi \textit{canary}, lalu menerapkan model bermasalah untuk memicu \textit{rollback}. & Model valid dipromosikan dan model bermasalah memicu \textit{rollback} otomatis tanpa kehilangan permintaan. \\
\hline
...
\hline
SK-N-04 & KNF-04 & Menginjeksikan \textit{fault} dengan Chaos Mesh (\texttt{pod-chaos} dan \texttt{network-chaos}) lalu mengamati pemulihan. & \textit{Workload} pulih otomatis dengan RTO dan RPO di bawah ambang yang ditetapkan. \\
\hline
...
\hline
SK-N-09 & KNF-09 & Mengakses seluruh konsol platform melalui satu sesi identitas dex serta memeriksa ketersediaan dokumentasi operasional pada repositori Gitea. & Seluruh konsol dapat dicapai melalui satu \textit{authentication flow} tanpa login ulang dan dokumentasi prosedur inti tersedia. \\
\hline
...
\hline
SK-N-12 & KNF-12 & Menjalankan \texttt{make usecase-crypto-test} untuk \textit{test coverage} dan \texttt{make phase-full} pada \textit{cluster} bersih sambil mengukur waktu konvergensi. & \textit{Test coverage} melewati ambang yang ditetapkan dan \texttt{phase-full} konvergen tanpa langkah manual. \\
\hline
\end{longtable}
}
