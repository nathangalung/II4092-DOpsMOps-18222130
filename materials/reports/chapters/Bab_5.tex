...
\input{tables/impl_layer_map.tex}
...
Konfigurasi \textit{cluster} Kafka dirangkum pada Tabel~\ref{tab:impl-kafka-config} agar pilihan teknis dan alasannya terbaca dalam satu pandangan. Keputusan pada tabel tersebut menjaga kontrak \textit{ingestion} tetap deklaratif, karena \texttt{KafkaTopic} dan \texttt{KafkaUser} CR membuat \textit{topic} beserta ACL dapat diaudit sebagai sumber daya Kubernetes, sedangkan kredensial SCRAM-SHA-512 mengalir dari OpenBao tanpa menyentuh manifes. Penskalaan konsumen mengikuti panjang antrean nyata karena KEDA membaca \textit{lag} dari Kafka Exporter melalui Prometheus. Penempatan \textit{broker} di luar \textit{mesh} Istio dengan enkripsi SASL\_SSL bawaan Kafka mengikuti rancangan titik masuk luar \textit{mesh} pada Subbab~\ref{subsec:lapisan-keamanan}, sehingga penyedia data tanpa \textit{sidecar} tetap terlayani tanpa melemahkan bawaan \textit{strict} internal.

\input{tables/impl_kafka_config.tex}
...
\input{tables/impl_minio_bucket.tex}
...
Verifikasi jalur eksekusi dilakukan dengan satu \textit{use case} domain nyata sebagai kasus pengujian, yaitu analitik pasar aset kripto, yang struktur layanan, konfigurasi, dan datanya dirinci pada Bab~\ref{chap:evaluasi}. Pemilihan domain ini berpijak pada tiga alasan yang dapat dirunut ke bukti. Pertama, \textit{use case} ini menjalankan seluruh sembilan \textit{layer} representatif yang ditetapkan pada Subbab~\ref{sec:layanan-industri}, dengan pemetaan tiap \textit{layer} ke komponennya dirinci pada Subbab~\ref{subsec:instans-verifikasi}, sehingga verifikasi fungsionalnya menyentuh himpunan layanan yang sama dengan yang dituntut praktik industri. Kedua, pasar aset kripto beroperasi terus menerus tanpa jam tutup bursa \autocite{makarov2020trading}, sehingga jalur \textit{ingestion} dan pemrosesan \textit{stream} teruji oleh aliran peristiwa nyata selama 24 jam penuh, bukan oleh beban sintetis yang dijadwalkan. Ketiga, volatilitas harga aset kripto yang tinggi dan bergerombol \autocite{katsiampa2017volatility} membuat distribusi fitur turunan harga rawan bergeser dalam hitungan jam, sehingga sub-sistem deteksi \textit{drift} dan \textit{retraining} otomatis memperoleh pemicu alami tanpa perlu injeksi buatan sebagai satu-satunya sumber pengujian. Pemilihan ini tidak mengubah sifat domain-agnostik dari platform, karena seluruh kode dan konfigurasi platform tetap berada pada repositori \texttt{platform/}, sedangkan kode dan konfigurasi spesifik domain tinggal pada direktori \textit{use case} tersendiri yang mengikuti pola \textit{base} dan \textit{overlays} \textit{Kustomize}, sehingga \textit{use case} lain cukup menyalin dan menyesuaikan berkas konfigurasi domain tanpa mengubah kode platform. Sifat lepas-domain kesembilan \textit{layer} itulah yang membuat pemilihan \textit{use case} ini tidak mengurangi keberlakuan platform pada domain lain.
