...
Bagian ini mengimplementasikan rancangan pada Bagian~\ref{sec:arsitektur-terintegrasi}. Komponen disusun ke dalam \textit{namespace} per \textit{layer} dan dihubungkan melalui kontrak \textit{API} yang stabil. Setiap \textit{namespace} memuat satu kelompok komponen fungsional sehingga kebijakan \textit{NetworkPolicy} dapat ditegakkan dengan model \textit{default-deny} dan pengecualian eksplisit per arah komunikasi. \textit{Deployment} komponen mengikuti pola GitOps melalui Argo CD, sedangkan eksekusi \textit{pipeline} CI mengikuti pola Tekton, dengan setiap komponen tunduk pada \textit{template} \textit{ApplicationSet} agar penambahan komponen baru tidak menambah \textit{boilerplate}.

\subsection{\textit{Data Ingestion Layer}}
...
\textit{Cluster} Kafka diatur dengan \textit{KRaft} murni tanpa ZooKeeper agar topologi tetap ringan. SASL\_SSL dipakai sebagai protokol keamanan antar klien dengan kredensial klien dikelola \textit{Strimzi} melalui \texttt{KafkaUser}. \textit{Topic} dideklarasikan sebagai \texttt{KafkaTopic} CR dan ACL dideklarasikan sebagai bagian dari \texttt{KafkaUser} sehingga keseluruhan kontrak \textit{ingestion} tersimpan sebagai sumber daya Kubernetes yang dapat di-\textit{audit}. Mekanisme SASL yang dipakai adalah SCRAM-SHA-512 dengan kredensial yang disebarkan External Secrets Operator dari OpenBao, dan topologi \textit{topic} bawaan memakai enam \textit{partition} dengan faktor replikasi satu pada satu \textit{node} yang dapat dinaikkan menjadi tiga pada multi-\textit{node}. \textit{Lag} antrean dipantau KEDA \texttt{ScaledObject} yang membaca metrik Kafka Exporter melalui Prometheus. PeerAuth \texttt{PERMISSIVE} diterapkan pada \textit{broker} agar penyedia data di luar \textit{mesh} masih dapat berkomunikasi tanpa terlebih dahulu menambahkan \textit{sidecar} Istio.

\subsection{\textit{Processing Layer}}
\label{subsec:impl-pemrosesan}

Pemrosesan \textit{stream} menggunakan operator Flink yang mengelola \textit{FlinkDeployment}. Pemrosesan \textit{batch} menggunakan operator Spark dan dijalankan sebagai \textit{SparkApplication}. Transformasi pada \textit{warehouse} menggunakan dbt yang dijalankan sebagai \textit{task} \texttt{KubernetesPodOperator} pada DAG Airflow \autocite{airflow2024}. Airflow mengorkestrasi \textit{pipeline} data berorientasi \textit{batch}, termasuk pembangunan \textit{gold layer}, sedangkan orkestrasi \textit{model training} ditangani oleh Kubeflow Pipelines.

Flink dipasang dengan \textit{session cluster} mode \textit{Standalone} dan tugas \textit{stream} dideklarasikan sebagai \texttt{FlinkSessionJob} CR sehingga setiap \textit{job} dapat di-\textit{deploy} secara independen melalui Argo CD. Spark Operator memanfaatkan \textit{webhook} mutasi untuk menyisipkan \textit{node selector} dan \textit{toleration} berdasarkan label \texttt{queue-name} yang ditetapkan oleh Kueue. dbt menjalankan model SQL pada ClickHouse dengan \texttt{profiles.yaml} yang merujuk rahasia dari OpenBao melalui ESO. Airflow memakai \textit{git-sync} \textit{sidecar} yang menarik DAG dari Gitea sehingga pembaruan DAG tidak memerlukan pembangunan ulang \textit{image}. \textit{Checkpoint} dan \textit{savepoint} Flink ditulis ke MinIO sehingga pemulihan \textit{job} tidak kehilangan keadaan, dan \textit{topic} hasil \textit{stream} dikonsumsi \textit{analytics layer} melalui tabel \textit{Kafka engine} ClickHouse.

\subsection{\textit{Storage Layer}}
\label{subsec:impl-penyimpanan}

\textit{Warehouse} kolumnar menggunakan ClickHouse \autocite{clickhouse2024} dengan operator ClickHouse pada satu \textit{shard}. \textit{Data lake} menggunakan MinIO sebagai \textit{object store} dan Apache Iceberg sebagai \textit{table format}, dengan Lakekeeper \autocite{lakekeeper2025} sebagai katalog Iceberg REST dan Trino \autocite{trino2024} sebagai \textit{query engine}. Penyimpanan kunci-nilai dan vektor menggunakan Valkey \autocite{valkey2025} dengan protokol RESP untuk akses berlatensi rendah, dan Qdrant \autocite{qdrant2025} sebagai basis data vektor untuk \textit{embedding} berdimensi tinggi. Penyimpanan relasional menggunakan PostgreSQL melalui operator CNPG dengan \textit{plugin} \textit{barman-cloud} untuk pencadangan ke MinIO, dan MySQL sebagai \texttt{Deployment} dengan \textit{persistent volume} pada \textit{storage class} lokal untuk komponen yang terikat pada protokol MySQL. Apache Superset \autocite{superset2025} disiapkan sebagai antarmuka BI untuk peran \textit{business user} dengan koneksi ke ClickHouse, Trino, dan MySQL melalui SQLAlchemy.

ClickHouse memakai \texttt{ClickHouseKeeper} sebagai pengganti ZooKeeper untuk koordinasi metadata replikasi. MinIO dipasang dengan empat \texttt{drive} pada satu \textit{node} untuk menjaga skema penamaan \texttt{erasure code} yang seragam ketika \textit{cluster} berpindah ke banyak \textit{node}. Lakekeeper menyediakan \texttt{Warehouse} dan \texttt{Project} REST API yang dipakai oleh Spark, Trino, dan Flink dengan menggunakan \textit{header} \texttt{x-project-id} sebagai pemisah antar \textit{warehouse}. lakeFS \autocite{lakefs2025} dipasang sebagai \textit{version control layer} di atas MinIO sehingga eksperimen pada cabang data dapat dijalankan tanpa mengganggu data utama. MinIO disusun dengan \textit{bucket} terpisah per kepentingan, antara lain \texttt{mlflow} untuk artefak eksperimen, \texttt{feast} untuk \textit{registry} berbasis berkas, \texttt{warehouse} untuk data \textit{lakehouse}, \texttt{flink-checkpoints} untuk \textit{checkpoint} \textit{streaming}, serta \texttt{velero-backups} untuk pencadangan, dengan kapasitas yang dipantau melalui metrik MinIO pada Prometheus. Qdrant dilengkapi \textit{persistent volume}, \texttt{PodDisruptionBudget}, dan kunci API yang disalurkan External Secrets Operator.

\subsection{\textit{Model Lifecycle Layer} dan \textit{Model Serving Layer}}
\label{subsec:impl-model}

MLflow \autocite{mlflow2024} dijalankan dengan basis data PostgreSQL dan \textit{artifact store} pada MinIO. Kubeflow Pipelines \autocite{kubeflow2024} dijalankan pada \textit{namespace} terpisah dengan dukungan multi-\textit{tenant}, dilengkapi Kubeflow Notebooks untuk lingkungan eksperimen interaktif dan Kubeflow Trainer \autocite{kubeflowtrainer2025} untuk pelatihan terdistribusi. KServe \autocite{kserve2024} dipakai untuk \textit{inference} dengan dukungan \textit{runtime} MLflow, scikit-learn, dan ONNX, dan dengan Knative Serving \autocite{knative2025} sebagai bidang \textit{serverless} bawaan yang menyediakan penskalaan ke nol. Argo Workflows berperan sebagai \textit{orchestrator} \textit{job} di luar Kubeflow Pipelines, sedangkan Argo Rollouts \autocite{argorollouts2025} mengelola promosi versi pada \textit{serving layer} dengan strategi \textit{canary} dan \textit{progressive analysis} berbasis metrik Prometheus.

Kubeflow Pipelines memakai MySQL sebagai \textit{metadata store} dan MinIO sebagai \textit{artifact store}, dengan \texttt{kfp-launcher} yang membungkus eksekusi langkah \textit{pipeline} agar autentikasi ke MinIO dilakukan menggunakan \texttt{secretAsEnv}. KServe \texttt{InferenceService} dideklarasikan pada \textit{namespace} \texttt{model-serving} dengan label \texttt{part-of=kserve} agar Kyverno tidak menolak \textit{Pod} \textit{mutator} bawaan. Kueue \autocite{kueue2024} mengelola antrean pelatihan dengan \texttt{ClusterQueue} per kelompok beban kerja sehingga sumber daya CPU dan GPU dapat dibagi adil antara \textit{tenant}. Argo Rollouts mengevaluasi metrik dari Prometheus pada setiap langkah \textit{canary} untuk menentukan apakah versi \textit{canary} layak dipromosikan menjadi versi \textit{stable}.
...
Implementasi menangani tiga kasus khusus yang sama dengan rancangan pada Subbab~\ref{subsec:kasus-khusus}. Ketiganya diwujudkan tanpa menambah komponen baru di luar \texttt{CronWorkflow} dan Argo Rollouts yang sudah terpasang. Perilaku tiap kasus dapat diperiksa dari manifes dan log eksekusi pada \textit{namespace} \texttt{model-lifecycle}. Rinciannya sebagai berikut.

\vspace{0.5em}
\begin{enumerate}
    \item \textbf{Jendela tunggu label per model}

    Jendela tunggu label diatur per model agar \textit{retraining} tidak dipicu sebelum label tersedia cukup.
...
Ketiga kasus tersebut ditangani tanpa percabangan tambahan pada \textit{workflow}. \texttt{retrain-on-drift} CronWorkflow terdiri atas dua langkah berurutan, yaitu pengukuran \textit{drift} dan langkah keputusan yang memicu \textit{retraining}, sedangkan penghentian promosi saat analisis \textit{canary} gagal ditangani oleh Argo Rollouts pada \textit{serving layer}. Ambang PSI dan KS beserta jendela \textit{lookback} dideklarasikan sebagai parameter pada CronWorkflow itu sendiri, sehingga perubahan kebijakan tetap melalui \textit{commit} Git dan dapat di-\textit{audit}. Nilai parameter yang berlaku dapat dibaca langsung dari manifes sehingga audit kebijakan tidak memerlukan akses ke sistem lain.

\section{Implementasi Sub-sistem Layanan Fitur \textit{Dual-Store}}
\label{sec:impl-feast}

Sub-sistem layanan fitur \textit{dual-store} mewujudkan rancangan pada Bagian~\ref{sec:feature-store-dual} dengan satu sumber definisi fitur dan satu kontrak Feast untuk \textit{tabular feature}, sedangkan fitur \textit{embedding} dikelola pada indeks vektor terpisah. \textit{Tabular feature} dimaterialisasi ke \textit{online store} Valkey dan diakses melalui API Feast, sedangkan indeks vektor Qdrant diisi dan diakses melalui klien \textit{native} di luar materialisasi Feast. Ketiga penyimpanan itu terikat pada satu sumber definisi di \texttt{feature\_repo/} sehingga perubahan skema fitur cukup dilakukan pada satu tempat. Bagian ini menjawab KF-01 sampai KF-04 yang menuntut manajemen fitur terpusat, \textit{serving} ganda \textit{online}-\textit{offline}, validitas temporal, dan dukungan \textit{vector embedding}.
...
Verifikasi jalur eksekusi dilakukan dengan satu \textit{use case} domain nyata sebagai kasus pengujian, yang struktur layanan, konfigurasi, dan datanya dirinci pada Bab~\ref{chap:evaluasi}. Pemilihan ini tidak mengubah sifat domain-agnostik dari platform, karena seluruh kode dan konfigurasi platform tetap berada pada repositori \texttt{platform/}, sedangkan kode dan konfigurasi spesifik domain tinggal pada direktori \textit{use case} tersendiri yang terpisah dari platform. Manifes Kubernetes domain mengikuti pola \textit{base} dan \textit{overlays} \textit{Kustomize} sehingga parameter spesifik lingkungan cukup disetel pada \textit{overlay layer} tanpa menyentuh definisi dasar. Pola yang sama berlaku untuk \textit{use case} lain, karena berkas konfigurasi domain cukup disalin dan disesuaikan tanpa mengubah kode platform. \textit{Use case} ini dipilih sebagai kasus pengujian karena menjalankan seluruh sembilan kelompok layanan representatif yang ditetapkan pada Subbab~\ref{sec:layanan-industri}, mulai dari \textit{data ingestion} dan pemrosesan data hingga \textit{model serving}, tata kelola, observabilitas, pengiriman GitOps, dan keamanan platform. Dengan cakupan tersebut, verifikasi fungsional menyentuh himpunan layanan yang sama dengan yang terbukti berulang pada praktik industri, bukan sekadar sebagian jalur. Sifat lepas-domain kesembilan kelompok itulah yang membuat pemilihan \textit{use case} ini tidak mengurangi keberlakuan platform pada domain lain.

\subsection{Lingkup Verifikasi}
\label{subsec:lingkup-verifikasi}

Verifikasi mencakup lima jalur yang masing-masing menyasar satu sub-sistem. Kelimanya dipilih dari alur ujung ke ujung \textit{use case} sehingga setiap kontrak antar \textit{layer} teruji oleh trafik nyata. Setiap jalur memiliki keluaran yang dapat diamati pada \textit{dashboard} atau tabel analitik sebagai bukti. Kelima jalur tersebut sebagai berikut.

\vspace{0.5em}
\begin{enumerate}
    \item \textbf{Jalur \textit{ingestion}}

    Jalur \textit{ingestion} diuji dengan \textit{stream} dari sumber eksternal yang masuk ke Kafka melalui layanan \texttt{websocket-collector} pada \textit{use case}.
    ...
    \item \textbf{Jalur \textit{training}}

    Jalur \textit{training} diuji dengan \textit{pipeline} Kubeflow yang membaca fitur dari Feast dan menulis model ke MLflow.

    \vspace{0.5em}

    \item \textbf{Jalur \textit{serving}}

    Jalur \textit{serving} diuji dengan \textit{InferenceService} KServe yang memuat model dari MLflow, dengan promosi versi pada \textit{serving layer} yang dikelola oleh Argo Rollouts.

    \vspace{0.5em}

    \item \textbf{Jalur \textit{retraining}}

    Jalur \textit{retraining} diuji dengan menjalankan \textit{drift-detector} dan memicu \textit{retrain-on-drift} \textit{workflow}.
\end{enumerate}

Uji ketahanan pendek menggunakan Chaos Mesh \autocite{chaosmesh2025} untuk menyuntikkan \textit{fault} pada \textit{pod} terpilih. Skenario yang dipakai adalah \texttt{PodChaos} yang mematikan \textit{pod} terpilih lalu mengamati pemulihannya oleh \textit{controller}. Pencadangan dijalankan terjadwal oleh Velero \autocite{velero2025} ke \textit{bucket} MinIO khusus pencadangan sebagai langkah disiplin pemulihan. Kedua mekanisme diuji kembali pada skenario ketahanan di Bab~\ref{chap:evaluasi}.

Lima jalur tersebut dipilih agar setiap sub-sistem mendapat verifikasi minimal satu skenario nyata, sehingga ketidaktepatan rancangan dapat ditemukan sebelum penilaian akhir pada Bab~\ref{chap:evaluasi}. Skenario \textit{use case} menetapkan beban yang lebih tinggi pada jalur \textit{ingestion} dan pemrosesan dibandingkan jalur \textit{training}, karena \textit{use case} ber-\textit{throughput} tinggi memproduksi banyak peristiwa per detik dari beberapa entitas secara paralel. Pengamatan dilakukan dengan \textit{dashboard} Grafana yang menampilkan metrik latensi, \textit{throughput}, panjang antrean Kafka, dan tingkat keberhasilan materialisasi. Hasil pengamatan dibandingkan dengan target metrik pada KNF Bab~\ref{chap:analisis-masalah} untuk menentukan apakah setiap jalur memenuhi target yang telah ditetapkan.

\subsection{Pengamatan Hasil Awal}
\label{subsec:pengamatan-awal}

Hasil pengamatan awal memperlihatkan bahwa jalur kendali platform terbentuk sesuai rancangan. Seluruh komponen pada kelima jalur berhasil di-\textit{deploy} dan direkonsiliasi oleh Argo CD, \textit{retraining control loop} terpicu otomatis ketika injeksi \textit{drift} terkendali diterapkan pada salah satu fitur, dan promosi versi pada \textit{serving layer} dikendalikan oleh Argo Rollouts sesuai konfigurasi \textit{canary}. Reproduksibilitas jalur diuji dengan merekonstruksi seluruh komponen platform dari Argo CD pada \textit{cluster} k3s yang baru, dan rekonstruksi tersebut berhasil dilakukan tanpa intervensi manual selain \textit{bootstrap} Argo CD awal. Penilaian kebenaran data \textit{end-to-end}, validitas \textit{point-in-time} pada pelatihan, latensi \textit{serving}, \textit{throughput} pemrosesan, dan ketepatan deteksi \textit{drift} diuraikan secara terperinci pada Bab~\ref{chap:evaluasi}.
