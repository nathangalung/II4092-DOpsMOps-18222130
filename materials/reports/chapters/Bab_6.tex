...
Evaluasi ini memiliki sejumlah keterbatasan yang perlu dicatat secara terbuka. Pertama, lingkungan pengujian adalah k3s node tunggal sesuai batasan B-2, sehingga klaim skalabilitas horizontal diukur melalui \textit{multi-replica} dan \textit{autoscaling} pada satu \textit{node}, bukan pada kluster multi-node. Kedua, deteksi \textit{drift} diverifikasi melalui injeksi \textit{drift} terkendali pada data nyata sehingga ketepatan terhadap pergeseran distribusi alami berdurasi panjang masih perlu diamati lebih lama, sedangkan karakterisasi beban penuh seperti latensi p99 dan \textit{throughput} puncak menjadi arah pengembangan pada lingkungan multi-node. Ketiga, sebagian skenario bergantung pada kelengkapan jalur data \textit{use case} sehingga penilaian kebenaran nilai fitur dilakukan setelah jalur ingestasi sampai \textit{gold} terisi penuh. Keterbatasan ini menjadi dasar saran tindak lanjut pada Bab~\ref{chap:penutup}, terutama untuk pengujian beban pada lingkungan multi-node dan pengamatan \textit{drift} pada data nyata berdurasi panjang.
