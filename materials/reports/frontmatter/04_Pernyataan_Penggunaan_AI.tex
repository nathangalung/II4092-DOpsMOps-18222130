% PERNYATAAN PENGGUNAAN AI
% Template: pernyataan jujur penggunaan \textit{tools} bantu AI selama penulisan.
%   - Baris yang Tidak digunakan ditulis "Tidak ada" pada kolom \textit{Tools} Bantu/Tingkat/Tempat.
%   - Baris yang digunakan menyebut nama \textit{tools}, tingkat (Rendah/Sedang/Tinggi), dan lokasi penggunaan.
\chapter*{PERNYATAAN PENGGUNAAN AI}
\addcontentsline{toc}{chapter}{PERNYATAAN PENGGUNAAN AI}
Saya yang bertanda tangan di bawah ini menyatakan bahwa dalam penulisan dokumen Tugas Akhir ini,
saya menggunakan \textit{tools} bantu kecerdasan artifisial (AI) untuk beberapa aspek tertentu di dalam proses penulisan,
seperti yang tercantum dalam tabel berikut.
\begin{table}[h]
\setlength{\tabcolsep}{3pt}
\begin{tabular}{|p{0.75cm}| p{4cm} | p{2.5cm} | p{2.5cm} | p{3cm} @{}| }
\hline
\centering\arraybackslash\textbf{No.} & \centering\arraybackslash\textbf{Jenis} & \centering\arraybackslash\textbf{\textit{Tools} Bantu} & \centering\arraybackslash\textbf{Tingkat} & \centering\arraybackslash\textbf{Tempat}\\
\hline
1 & Memeriksa ejaan dan tata bahasa & Claude & Tinggi & Semua bab \\
\hline
2 & Pembuatan teks & Tidak ada & Tidak ada & Tidak ada \\
\hline
3 & Pembuatan grafik, gambar, atau ilustrasi & Tidak ada & Tidak ada & Tidak ada \\
\hline
4 & Pencarian informasi atau referensi & Claude & Sangat tinggi & Bab 1 - Bab 5 \\
\hline
5 & Penyusunan bahan diskusi & Claude & Sangat tinggi & Semua bab \\
\hline
6 & Penyusunan kode program & Claude & Sedang & Bab 5 dan Lampiran B \\
\hline
7 & Penyusunan formula atau perhitungan matematis & Tidak ada & Tidak ada & Tidak ada \\
\hline
8 & Penyusunan konsep desain atau algoritma & Claude & Rendah & Bab 4 \\
\hline
9 & Penyusunan analisis data & Tidak ada & Tidak ada & Tidak ada \\
\hline
10 & Penyusunan kesimpulan atau rekomendasi & Tidak ada & Tidak ada & Tidak ada \\
\hline
\end{tabular}
\end{table}


Bandung, \today\\
\\[2cm]

\underline{Bryan P. Hutagalung} \\
NIM: 18222130
