...
- Placement spec is `\begin{table}[!htb]` for EVERY table (user directive 2026-07-12 Round 17: no blank space at page bottoms — if a table does not fit the remaining page, the next paragraph flows up to fill the page and the table lands at the top of the following page; `h` still puts it directly under the referencing paragraph whenever it fits, so the earlier attach-under-paragraph behavior survives in the common case). The former all-`[H]` policy (previous directive) is RETIRED: `[H]` forced a page break with a blank bottom whenever the table did not fit, which the user explicitly rejected. Do NOT add `\usepackage{float}` — floatrow errors on it and skips itself, breaking `\floatsetup`. Multi-page tables stay `longtable` (they start on the current page and continue onto the next — the user's other accepted behavior). Float acceptance is tuned in `report.tex` (`\topfraction` 0.9, `\bottomfraction` 0.6, `\textfraction` 0.07, `\floatpagefraction` 0.85). The mid-section `\clearpage` removals stand. FIGURES stay on `[!htb]`/`[!htbp]`; the three MLOps level figures keep `[!htb]` inside the MLOps subsection (II.1.2, maturity block), in level order 0, 1, 2. EXEMPTION to the no-blank rule (user directive 2026-07-12: "bab tidak boleh mulai di halaman genap"): chapters — including the DAFTAR lists, DAFTAR PUSTAKA, and each LAMPIRAN — must open on an odd page, so the inserted blank filler pages before chapter openers are mandatory and must NOT be removed (mechanism: `book` `openright` + `twoside` in the mainmatter `\newgeometry` + the explicit `\cleardoublepage` before DAFTAR PUSTAKA; do not add `openany`). The only removed blank is the trailing page after Lampiran B: `\backmatter` in `report.tex` is commented out because no chapter follows it. Other accepted residual gaps: longtable pages whose next row is too tall to fit (rows are unbreakable; the table itself already continues on the next page), and float-tail pages where the next block (a longtable or another float) cannot start in the small remainder.
- Header row cells are horizontally centered in EVERY table, main matter and frontmatter alike (user directive 2026-07-12 Round 17: "make sure all the header table is in the middle or center (just the header)"). Center via the column spec (`>{\centering\arraybackslash}m{...}`), via `\multicolumn{1}{|>{\centering\arraybackslash}m{...}|}{...}`, or — for header cells sitting in raggedright body columns — a per-cell `\centering\arraybackslash` prefix on the header row only. Body cells stay raggedright. Longtables need the fix in both the `\endfirsthead` and the `\endhead` copy. Spanning section rows inside the body (`\multicolumn{4}{|l|}{\textbf{...}}`) are body content, not headers, and stay left-aligned.
- Captions (table, figure, listing) never contain count words — `Pemetaan Sembilan Layer`, `Tiga Tahun Tiga Skenario`, `Empat Kerangka` are banned (user directive 2026-07-12 Round 17). Reword to the classifying noun: `Pemetaan \textit{Layer} Platform ...`, `... per Skenario`. Proper names and fixed designations stay: `MLOps Level 0`, `KServe v2`, a price-snapshot date such as `per 16 Juni 2026`.
...
### 4.x+ Sentence openers: no leading number words (Round 17, 2026-07-12)

No sentence anywhere in the report (prose, enumerate explanations, table
footnotes) may START with a spelled-out number word: cardinals (Satu, Dua,
Tiga, Empat, Lima, Enam, ...), determiner ordinals (Kedua/Ketiga/Keempat +
noun, as in "Ketiga tekanan tersebut"), pronoun forms (Keduanya, Ketiganya),
or bare sequence markers ("Pertama, ...", "Kedua, ..."). The user reads
number-first sentences as AI-generated. Restructure noun-first and keep the
count inside the sentence, or drop it when redundant:
"Empat pilar operasi DataOps adalah X" -> "Operasi DataOps bertumpu pada
empat pilar, yaitu X"; "Ketiganya menjaga fokus" -> "Batasan-batasan itu
menjaga fokus". Counts INSIDE sentences stay fine. Do not solve it by
opening everything with Terdapat (max 2 per chapter) and never with Adapun.
Verify with: `grep -nE '(^|[.!?] )(Satu|Dua|Tiga|Empat|Lima|Enam|Tujuh|Delapan|Sembilan|Sepuluh|Kedua|Ketiga|Keempat|Kelima|Keenam)[a-z-]* ' chapters/*.tex tables/*.tex appendices/*.tex frontmatter/*.tex` (also scan
after `{\footnotesize ` — the anchored pattern misses that position).
