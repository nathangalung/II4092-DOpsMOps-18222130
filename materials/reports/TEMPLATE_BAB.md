...
- ATURAN BEBAS-ALAT BAB IV (revisi 2026-07-11): tidak ada nama alat *open source* pada Bab IV sebelum §IV.3 (`sec:pemilihan-tools`); IV.1 dan IV.2 berbicara pada level layanan dan layer (Kubernetes dikecualikan karena premis judul tugas akhir; contoh alat per layanan hanya boleh pada paragraf penutup unit Bab II). IV.1 menyebut tujuh komponen wajib MLOps level 2 secara generik dengan penunjuk tiga langkah (layanan IV.2, pemilihan IV.3, pemetaan IV.4); kalimat pemetaan komponen→alat hidup pada akhir paragraf derivasi pembuka IV.4. Urutan sembilan layer = urutan subbab konsep Bab II (`sec:kubernetes`, `sec:manajemen-data` menaungi dua layer ingestion+processing dan storage, `sec:feature-store`, `sec:siklus-model`, `sec:tata-kelola-teori`, `sec:observability`, `sec:gitops`, `sec:keamanan-platform`); `sec:drift` bersifat lintas layer dan diwujudkan sebagai sub-sistem IV.6 (`sec:drift-ct`); pemetaan ini dinyatakan eksplisit pada prosa IV.2.2 dan diikat lagi pada pembuka IV.4. IV.3 dan IV.4 memakai sembilan subbab dengan judul layer yang identik (satu taksonomi, tanpa konsolidasi perantara); IV.4 subbab kelima (Model Lifecycle Layer) memuat bagian model serving sesuai baris 5 tabel dan membawa dua gambar.
- Pemetaan peran pengguna ke antarmuka generik dan *layer* (tabel `peran_akses`, TANPA kolom nama alat) ditempatkan di IV.1 Gambaran Umum sebagai orientasi “siapa yang memakai platform”, bukan sebagai subbab tersendiri di bawah IV.4. Keputusan ini menurunkan bingkai per-peran dari rancangan ke orientasi: IV.4 berisi murni rancangan sembilan *layer* (T-1), tanpa subbab “Pandangan per Peran”. Identitas/SSO bersama dijelaskan sekali di IV.5.3 (identitas-akses), tidak diulang di IV.1. Tidak ada gambar penuh-halaman per peran di Bab IV. Lampiran fragmentasi per peran (dahulu Lampiran C) telah dihapus karena tesis konsisten bersifat *layer-centric*: masalah dirumuskan sebagai kontrak antar *layer*, KF/KNF dipetakan ke *layer*, rancangan Bab IV memuat sembilan *layer*, dan diagram *after* terorganisasi per *layer*, sehingga diagram *before* per peran menjadi satu-satunya artefak yang terorganisasi per peran dan menimbulkan ketaksesuaian taksonomi *before*/*after* yang rawan dipersoalkan penguji. Sebagai gantinya, fragmentasi kini ditampilkan sebagai dua alur layanan terfragmentasi (Gambar III.1 untuk DataOps dan Gambar III.2 untuk MLOps) yang tersusun per alur, bukan per peran, namun tetap memuat keempat peran sebagai aktor sehingga rincian per peran tidak diperlukan dan taksonomi *before*/*after* sama-sama berbasis layanan. Bingkai per-peran pada Bab III ditulis kebutuhan-dahulu (peran sebagai tempat kebutuhan paling terasa), bukan peran-dahulu.

Landasan sembilan *layer* (revisi 2026-07-11, taksonomi tunggal): kesembilan *layer* pada IV.2, IV.3, dan IV.4 adalah SATU pemisahan yang sama, bukan taksonomi baru, karena tiap *layer* berpijak pada baris bersumber Tabel `layanan_industri` dan mengikuti urutan subbab konsep Bab II. Anotasi kategori per *layer* dinyatakan pada paragraf derivasi pembuka IV.4 dari enam kategori najafabadi2024analysis (*data curation*, *storage and versioning*, *ML training*, CI/CD, *inference*, *infrastructure and supporting services*) plus kreuzberger2023mlops dan tuntutan tata kelola DataOps munappy2020adhoc; Model Lifecycle Layer menampung kategori *inference* pada bagian *model serving*-nya sesuai baris kelima tabel. Jangan pernah menyebut sembilan *layer* sebagai standar industri; selalu sebagai klasifikasi komponen bersumber yang dapat dirunut. Kesepuluh berkas `.eraser` Layer_* membawa anotasi kategori sumber pada label tiap grup, dan caption Gambar `fig:integrate-service` (Subbab IV.2) mengaitkan tiap *layer* ke sumbernya melalui Tabel `layanan_industri`; perubahan derivasi wajib disinkronkan ke label diagram Layer_* dan tabel tersebut (daftar lengkap pada `diagrams/README.md` aturan 8). Bukti komponen dari kreuzberger2023mlops, amershi2019software, najafabadi2024analysis (termasuk frekuensi kemunculan komponen pada 43 studi: model repository 21, ML metadata repository 15, ML experiment pipeline 14; tanpa klaim mayoritas mutlak), googlecloud2024mlops, dan salama2021practitioners (kapabilitas teknis inti MLOps) beserta sisi data munappy2020adhoc, rella2022mlops, dan jain2025integrating disajikan pada IV.2 (`sec:layanan-industri`, tabel `layanan_industri`); lead-in IV.3 merujuk klasifikasi tersebut, dan kesembilan *layer* yang sama menjadi sembilan subbab perancangan pada IV.4 (judul subbab IV.3 dan IV.4 identik). Justifikasi cakupan *use case* menunjuk kesembilan *layer* itu pada Bab V (`sec:verifikasi`) dan Bab VI (`subsec:instans-verifikasi`). Abstrak menyebut kesembilan *layer* dengan nama yang sama persis dengan IV.4. Sejak register round 8 (nama disesuaikan taksonomi tunggal 2026-07-11): seluruh nama layer memakai bentuk English penuh dengan urutan English (Orchestration and Infrastructure Layer, Data Ingestion and Processing Layer, Data Storage Layer, Feature Service Layer, Model Lifecycle Layer, Data Governance Layer, Observability Layer, GitOps and Continuous Delivery Layer, Platform Security Layer; subbab Bab V penyimpanan memakai Data Storage Layer), bidang kendali menjadi *control plane*, ingestasi menjadi *ingestion*/*data ingestion*, penyajian model menjadi *model serving*, siklus hidup model menjadi *model lifecycle*, pelatihan ulang menjadi *retraining*; padanan lengkap ada pada `WRITING_GUIDE.md` blok Round 8.
