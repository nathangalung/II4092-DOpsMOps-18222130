...
## Common / service mesh
istio;              1.29.2 (bumped 2026-07-05 from 1.28.6; PeerAuthentication STRICT mesh-wide, default AuthorizationPolicy deny, DestinationRule TLS 1.3). Control-plane images and labels at 1.29.2; CRDs regenerated from istioctl 1.29.2 (14 CRDs, all with resource-policy: keep). istio-install.yaml is vendored istioctl output with load-bearing hand edits, forward-ported: (1) mesh-ConfigMap extensionProviders registering oauth2-proxy as envoyExtAuthzHttp — required by action: CUSTOM AuthorizationPolicies (ADR-024); (2) accessLogFile: /dev/stdout + tcpKeepalive; (3) sidecar-injector busybox:1.37 override (ADR-006). A stock istioctl re-render is not reproducible (CNIConfig --set cniNamespace schema drift confirmed at 1.28.6), so treat istio-install.yaml as vendored and forward-port the edits on any future bump. Live apply needs mesh-auth verification (ext_authz to oauth2-proxy).
